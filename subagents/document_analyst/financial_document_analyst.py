...
def get_financial_document_tool() -> BaseTool:
    return Tool(
...
def get_financial_document_answer(query: str) -> str:
    ...
    for f in DATA_DIR.glob("*.pdf"):
        ...
        if (
            quarter in f.name.lower()
            and year in f.name.lower()
            and any(
                company.lower() in f.name.lower() for company in potential_companies
            )
        ):
            target_filename = f
            break
    ...
    if not target_filename:
        return (
            f"Tool Error: I couldn't find a PDF in my data folder for {quarter} {year}."
        )
    ...
    file_to_upload = None
    try:
        file_to_upload = genai.upload_file(
            path=target_filename, display_name=f"{quarter} {year} Report"
        )
        ...
        prompt_parts = [
            file_to_upload,
            "You are an expert financial analyst. Please answer the user's question based *only* on the contents of this file.",
            f"\nUser Question: {query}",
        ]
